        ...
        async function getRecommendationsOnly(requestData) {
            ...
            try {
                ...
                if (!result.success) {
                    throw new Error(result.message || '推荐API调用失败');
                }
                
                const recommendations = result.data.recommendations || [];
                console.log('✅ 推荐API调用完成，收到推荐数量:', recommendations.length);
        ...
        async function showRecommendationsSequentially(recommendations) {
            ...
            console.log("🔍 前端接收到的推荐数据:", JSON.stringify(recommendations, null, 2));
        ...
        async function showSimpleRecommendation(recommendation, index) {
            ...
            console.log(`🔍 第${index + 1}个推荐的详细数据:`, JSON.stringify(recommendation, null, 2));            
        ...
        async function displayRecommendationWithTypewriter(recommendation, index) {
            ...
            // 解析推荐数据
            const cocktailNameCn = recommendation.name?.chinese || recommendation.chinese ;
            const cocktailNameEn = recommendation.name?.english || recommendation.english ;
        ...
        async function getRecommendation() {
            // 收集特殊要求 - 兼容两种ID
            const specialRequirementElement = document.getElementById('specialRequirement') || document.getElementById('specialRequirements');
            userInput.special_requirements = specialRequirementElement ? specialRequirementElement.value : '';
        ...
        function fillRecommendationContent(recommendation, index) {
            ...
            // 🚨 严格数据验证 - 如果数据无效直接报错，不使用降级
            if (!recommendation) {
                console.error(`❌ 推荐数据为空: index=${index}`);
                throw new Error(`推荐数据 ${index} 为空`);
            }
            
            if (!recommendation.name || (!recommendation.name.chinese && !recommendation.chinese)) {
                console.error(`❌ 推荐 ${index} 缺少中文名称:`, recommendation);
                throw new Error(`推荐 ${index} 缺少中文名称`);
            }
            
            if (!recommendation.name || (!recommendation.name.english && !recommendation.english)) {
                console.error(`❌ 推荐 ${index} 缺少英文名称:`, recommendation);
                throw new Error(`推荐 ${index} 缺少英文名称`);
            }
            
            if (!recommendation.reason) {
                console.error(`❌ 推荐 ${index} 缺少推荐理由:`, recommendation);
                throw new Error(`推荐 ${index} 缺少推荐理由`);
            }
            
            console.log(`✅ 推荐 ${index} 数据验证通过`);
        ...
        function displayRecommendationInCard(index, recommendation) {
            ...
            // 🔧 修复数据结构处理：检查推荐数据的实际结构
            let actualRecommendation;
            if (recommendation && recommendation.content && recommendation.content.recommendations && recommendation.content.recommendations[0]) {
                // 流式API的数据结构：{ content: { recommendations: [actualData] } }
                actualRecommendation = recommendation.content.recommendations[0];
                console.log('📦 使用流式API数据结构:', actualRecommendation);
            } else if (recommendation && recommendation.name) {
                // 直接推荐数据结构：{ name: {...}, reason: "...", ... }
                actualRecommendation = recommendation;
                console.log('📦 使用直接推荐数据结构:', actualRecommendation);
            } else {
                console.error('❌ 无法识别的推荐数据结构:', recommendation);
                return;
            }
            ...
            if (contentElement && actualRecommendation) {
                // 停止调制动画
                try {
                    completeMakingAnimation(index);
                } catch (e) {
                ...
                // 构建完整内容 - 使用actualRecommendation
                console.log('🔍 推荐数据结构:', actualRecommendation);
                const cocktailName = actualRecommendation.name?.chinese || actualRecommendation.chinese ;
                const englishName = actualRecommendation.name?.english || actualRecommendation.english ;
                const reason = actualRecommendation.reason || '为你特别调制';
                const ingredients = actualRecommendation.recipe?.ingredients || actualRecommendation.ingredients || [];
                ...
                if (cardElement) {
                    ...
                    cardElement.onclick = () => {
                        showCocktailDetails({
                            ...
                            reason: reason,
                            ingredients: ingredients,
                            tasteProfile: actualRecommendation.taste_profile || '口感独特，值得品尝',
                            prepTime: actualRecommendation.prep_time || '5分钟',
                            difficulty: actualRecommendation.recipe?.difficulty || '简单',
                            alcoholContent: actualRecommendation.alcohol_content || '中度',
                            instructions: actualRecommendation.instructions || ['将材料混合', '充分摇匀', '倒入杯中'],
                            visual: actualRecommendation.visual || '色泽诱人',
                            bestTime: actualRecommendation.best_time || '任何时候'
                        });
                    };
                }
                
                console.log(`✅ 推荐卡片 ${index} 显示完成`);
            } else {
                console.error(`❌ 无法显示推荐卡片 ${index}:`, {
                    hasContentElement: !!contentElement,
                    hasActualRecommendation: !!actualRecommendation,
                    recommendation
                });
            }
        }
        ...
        function resetUIState() {
            ...
            // 强制清理所有可能的分析文本残留
            const allAnalysisElements = [
                'shakerAnalysisText',
                'analysisContent', 
                'analysisTitle'
            ];
            
            allAnalysisElements.forEach(elementId => {
                const element = document.getElementById(elementId);
                if (element) {
                    element.innerHTML = '';
                    element.textContent = '';
                    console.log(`🧹 已强制清理元素: ${elementId}`);
                }
            });
        ...
        window.startRecommendation = startAnotherDrink; // 重新推荐使用startAnotherDrink
        window.startAnotherDrink = startAnotherDrink; // 重新探索按钮
